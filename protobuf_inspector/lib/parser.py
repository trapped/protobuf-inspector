...
class Parser(object):
    ...
    def hex_dump(self, file, mark=None):
        lines = []
        offset = 0
        decorate = lambda i, x: x if (mark is None or offset + i < mark) else dim(x)
        while True:
            ...
            printable_chunk = "".join(
                decorate(i, chr(x) if 0x20 <= x < 0x7F else fg3("."))
    ...
    def safe_call(self, handler, x, *wargs):
        ...
        try:
            ...
            return "%s: %s%s" % (
                fg1("ERROR"),
                self.indent(format_exc()).strip(),
                self.indent(hex_dump),
            )
