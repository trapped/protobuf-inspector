...
class StandardParser(Parser):
    ...
    def parse_message(self, file, gtype, endgroup=None):
        ...
        while True:
            ...
            if wire_type == 3:
                ...
                x = self.safe_call(
                    lambda x: self.match_handler(type, wire_type)(x, type), x
                )
        ...
        if len(lines) <= self.message_compact_max_lines and self.to_display_compactly(
            gtype, lines
        ):
    ...
    def parse_packed(self, file, gtype):
        ...
        if len(lines) <= self.packed_compact_max_lines and self.to_display_compactly(
            gtype, lines
        ):
            return "[%s]" % (", ".join(lines))
        return "packed:\n%s" % (self.indent("\n".join(lines)))
